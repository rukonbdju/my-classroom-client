            ...
            <div className="relative group">
              ...
              <div className="absolute hidden  group-focus-within:block right-0 bg-slate-300 p-2 rounded-md">
                ...
                {user?.photoURL ? (
                  ...
                  <div className="p-3 rounded-full w-10 h-10 bg-blue-600 flex items-center justify-center">{user?.displayName?.slice(0,1)}</div>
                )}
                <h2 className="text-xl font-bold">{user?.displayName}</h2>
                <h2 className=" text-sm font-bold bg-slate-200 rounded-full px-2">{user?.email}</h2>
